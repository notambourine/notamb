/*!
 * notamb — sticky A/B bucketing that reports to your analytics
 * © 2026 NoTambourine — MIT license
 * https://notamb.notambourine.com · https://github.com/notambourine/notamb
 */
// Canonical source. `npm run build` slices the #region blocks below into
// every copy-paste variant served at https://notamb.notambourine.com — pick
...
// One entry per tracker; notamb() fans each exposure out to all of them.
const emitters: Emitter[] = [];
// #endregion

// #region emitter:ga4
// GA4 — a plain dataLayer push, readable by gtag.js and GTM alike. Register
// notamb_test / notamb_variant as custom dimensions (or map them in GTM) to report
// on ab_assigned.
// https://developers.google.com/analytics/devguides/collection/ga4/integration
emitters.push((test, variant) => {
  const w = window as any;
  w.dataLayer = w.dataLayer || [];
  w.dataLayer.push({ event: 'ab_assigned', notamb_test: test, notamb_variant: variant });
});
// #endregion
...
emitters.push((test, variant) => {
  ...
  try {
    if (sessionStorage.getItem('notamb:cart:' + test) === variant) return;
  } catch {
  ...
  cartTimer = setTimeout(() => {
    ...
    for (const t in cartWrites) attributes['__notamb_' + t] = cartWrites[t];
    fetch(root + 'cart/update.js', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      // Attributes merge, so sending only notamb keys leaves your own alone.
      ...
      .then((res) => {
        ...
        if (!res.ok) return;
        try {
          for (const t in cartWrites) sessionStorage.setItem('notamb:cart:' + t, cartWrites[t]);
        } catch {
 ...
 * Uniform random, sticky via localStorage ('notamb:' + test). Returns the
 * 0-based variant index for your code to render against.
 */
function notamb(test: string, variants: number = 2): number {
  const key = 'notamb:' + test;
  let v = NaN;
  try {
...
function useNotamb(test: string, variants: number = 2): number {
  return useState(() => notamb(test, variants))[0];
}
// #endregion

// #region export
export { notamb, useNotamb };
// #endregion
